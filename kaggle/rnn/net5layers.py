...
class Model(torch.nn.Module):
    def __init__(self, in_vocab, out_vocab, hidden_size):
        ...
        self.lstm = torch.nn.LSTM(in_vocab, hidden_size, bidirectional=True, num_layers=5,dropout=0.2).to(DEVICE)
...
def train_epoch_packed(model, optimizer, train_loader, n_epoch):
    ...
    if n_epoch > 0 and (n_epoch + 1) % 2 == 0:
        modelpath = "saved_models/{}.pt".format(str(jobtime) + "_" + str(n_epoch))
