...
class BatchNorm(object):
    ...
    def backward(self, delta):
        ...
        # print("self.dbeta: {}".format(self.dbeta))
        # print("NORM: {}".format(self.norm.shape))
        self.dgamma = np.sum(delta * self.norm, axis=0)
        # print("self.dgamma: {}".format(self.dgamma))
        # print("Dgamma: {}".format(self.dgamma))
        dvar_sqr = -0.5 * np.sum(dnorm * (self.x - self.mean) * np.power((self.var_sqr + self.eps), -3 / 2), axis=0)
        # print("dvar_sqr:", dvar_sqr)
        ...
        # print("dmiu: {}".format(dmiu))
        dw = dnorm * np.power((self.var_sqr + self.eps), -1 / 2) + dvar_sqr * (
        ...
        if self.nlayers == 1:
            ...
            for i in range(len(self.W) - 1, 0, -1):
                ...
                if (nb_layer == i and i != 0):
                    ...
                    # print(self.activations[0].derivative() * dz_prev)
                    ...
                    # cur_z_prime = np.multiply(self.input.tr, tmp)
                    # print("cur_Z_prime:{}".format(cur_z_prime))
                    nb_layer -= 1
                    print("BATCH END")
                else:
                    cur_z_prime = np.multiply(y_prime, self.activations[i - 1].derivative())
